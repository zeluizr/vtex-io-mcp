...
- [Portal migration redirect strategy](#portal-migration-redirect-strategy)
...
9. Once PR is approved, apply the merge to update the navigation sidebar.
   ...
   > For new information to appear in the Help Center, Education & Documentation team leaders must run the portal's build.
...
> See also: [Portal migration redirect strategy](#portal-migration-redirect-strategy).

#### Creating a redirect

This portal uses a [Netlify edge function](https://docs.netlify.com/build/edge-functions/overview/) to handle the complexity of redirecting users from our legacy portal. Therefore, to create a new redirect, you must edit the `public/redirects.json` file:
...
4. Add a new redirect object to the array following this format:
   ...
   **Example:**
   ...
   ```
   > If no Contentful article ID is provided, the function will match the article slug.
   > If no locale is provided, the redirect function may default to EN.
...
## Portal migration redirect strategy
