...
source: "https://developers.vtex.com/docs/guides/vtex-io-documentation-1-basicsetup"
---
...
> The `vtex use` command ensures that all your operations run in the specified workspace. To switch back to the main workspace (`master`), run `vtex use master` in the VTEX IO CLI.

To confirm the `account` and `workspace` you're using, run the `vtex whoami` command in your terminal.
